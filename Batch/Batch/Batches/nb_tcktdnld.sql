...
O_ERROR_MESSAGE       VARCHAR2(32000):= null;
     ...
     select order_no, master_po_no,SUPPLIER from rms.ordhead oh
        where oh.status ='A' and CREATE_DATETIME>= to_date('01-JAN-2024 13:30', 'DD-MON-YYYY hh24:mi') and rownum<='5000'
...
select * from int_asos.int_tckt_dnld_stage;
select * from int_asos.int_tckt_dnld_stage where order_no in (select order_no from rms.ordhead where trunc(CREATE_DATETIME) = trunc(sysdate));
