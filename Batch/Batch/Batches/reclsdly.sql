...
select * from ma_asos.MA_STG_NEW_BUY_HIER;

select STATUS, THREAD_NUMBER, ERROR_MSG,count(1) from ma_asos.MA_STG_NEW_BUY_HIER group by STATUS, THREAD_NUMBER, ERROR_MSG;


select LAST_UPDATE_ID,count(1) from ma_asos.ma_item_attributes group by LAST_UPDATE_ID ;
select count(1) from ma_asos.ma_item_attributes where trunc(LAST_UPDATE_DATETIME)> ='18-FEB-22'; --97 31 011
...
SELECT COUNT(1)
       ...
       WHERE rh.reclass_no   = ri.reclass_no
         AND rh.reclass_date =NVL(TO_DATE('20240115', 'YYYYMMDD'), rh.reclass_date)
         AND (ri.item = im.item
...
select * from reclass_head where reclass_no between 7110 and 8000;
...
select count(1) from reclass_item where reclass_no in (select reclass_no from reclass_head where RECLASS_DATE >= '09-NOV-21');
select count(1) from reclass_head where RECLASS_DATE >= '09-NOV-21';
