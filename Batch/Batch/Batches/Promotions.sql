...
create table ma_stage_simple_promo_bk as  select * from ma_asos.ma_stage_simple_promo;
...
select item,count(1) from ma_asos.ma_stage_simple_promo where status='N';

create table ma_stage_simple_promo_bk_PEE as select distinct item from ma_asos.ma_stage_simple_promo where status='N';
insert into skumar.ma_stage_simple_promo_bk_PEE select distinct item from ma_asos.ma_stage_simple_promo where status='N';
...
select START_DATE,count(1) from rms.rpm_promo_dtl where START_DATE between '16-JAN-24' and '20-JAN-24' and state ='3' group by START_DATE order by 1;
select START_DATE,count(1) from rms.rpm_promo_dtl where START_DATE between '16-JAN-24' and '17-JAN-24'  and state!='3' group by START_DATE order by 1;
...
delete from ma_asos.ma_stage_simple_promo where status ='N' and rownum <= '10000';
select * from ma_asos.ma_stage_simple_promo where status ='N'; --
           ...
           rpd.end_date as dtl_end_date,rp.promo_display_id
			 ...
			 where rp.promo_id = rpc.promo_id
			   ...
			   and rpd.promo_dtl_id = rpdmn.promo_dtl_id
                ...
                and trunc(rp.start_date) = '16-JAN-2024'
--                and rpzl.location ='20001'
                and rpdl.promo_dtl_list_grp_id = rpdlg.promo_dtl_list_grp_id
                --and rp.PROMO_EVENT_ID ='58'
                AND rownum <= '2000'
                and rpddl.promo_dtl_list_id = rpdl.promo_dtl_list_id 
--              and not exists (select 1 from ma_asos.ma_stage_simple_promo mpc where mpc.item = rpdmn.item) order by promo_start_date,item,location;
                and not exists (select 1 from skumar.ma_stage_simple_promo_bk_PEE mpc where mpc.item = rpdmn.item) order by promo_start_date,item,location;
...
cursor c_get_item_sp (l_dept rms.subclass.dept%type, l_class rms.subclass.class%type, l_subclass rms.subclass.subclass%type, l_zone_id rms.rpm_stage_simple_promo.zone_id%type,l_location rms.rpm_stage_simple_promo.location%type)is
   ...
   select distinct im.item,ma.zone_id,ma.store as location,'Promotion: ' as P_name,s.currency_code,il.selling_unit_retail as current_retail,il.SELLING_UOM    
                 from skumar.item_master_op     im,
                        ...
                        and not exists (select 1 from rms.rpm_promo_item_loc_expl rpl where rpl.item = im.item and rpl.DETAIL_START_DATE >= '01-JAN-2024' and  rpl.DETAIL_END_DATE <= '30-JAN-2024')
                        ) where rownum <= '20';
...
For m in 1..2 loop
     ...
     select  vdate+10 into l_promo_start_date from rms.period; 
     select  vdate+30+m into l_promo_end_date from rms.period; 
     select  vdate+10 into l_dtl_start_date from rms.period; 
