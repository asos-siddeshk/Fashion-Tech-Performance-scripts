...
select count(1) from IF_TRAN_DATA;      --2957930 
select count(1) from TRAN_DATA;         --9418945

select count(1) from TRAN_DATA;         --9458085
select count(1) from TRAN_DATA;         --9 490 249


select * from TRAN_DATA; --7608180
select * from IF_TRAN_DATA; --20180142
select * from TRAN_DATA_history where TRAN_DATE=''; --20180142

select * from IF_TRAN_DATA_BK_RETEN;

delete from TRAN_DATA_a where TRAN_DATE= '10-MAY-21';
delete from TRAN_DATA_b where TRAN_DATE= '10-MAY-21';
commit;
...
select * from IF_TRAN_DATA_BK_RETEN; --20180142

update IF_TRAN_DATA_BK_RETEN set TRAN_DATE=TRAN_DATE+1;


select distinct tran_date,count(1) from IF_TRAN_DATA_BK_RETEN where tran_date!='09-NOV-21' group by tran_date order by 1; --8058150
select distinct tran_code,count(1) from IF_TRAN_DATA_BK_RETEN where tran_date!='09-NOV-21' group by tran_code order by 1; --8058150
...
select distinct tran_date,count(1) from TRAN_DATA_a group by tran_date order by 1; --8058150
select distinct tran_code,count(1) from TRAN_DATA_a group by tran_code order by 1; --8058150
...
select distinct tran_date,count(1) from TRAN_DATA_history where TRAN_DATE ='06-MAR-22' group by tran_date order by 1; --8058150
...
SELECT 
ITEM, DEPT, CLASS, SUBCLASS, PACK_IND, LOC_TYPE, LOCATION,TRAN_DATE+1,TRAN_CODE, ADJ_CODE, UNITS, TOTAL_COST, TOTAL_RETAIL, REF_NO_1, REF_NO_2, GL_REF_NO, OLD_UNIT_RETAIL, NEW_UNIT_RETAIL, PGM_NAME, SALES_TYPE, VAT_RATE, AV_COST, TRAN_DATA_TIMESTAMP+1 as TIMESTAMP, REF_PACK_NO, null as TOTAL_COST_EXCL_ELC,null,null  FROM IF_TRAN_DATA_BK_RETEN
where tran_code in ('30','31','32','33','37','38');

insert into tran_data_a (ITEM, DEPT, CLASS, SUBCLASS, PACK_IND, LOC_TYPE, LOCATION, TRAN_DATE, TRAN_CODE, ADJ_CODE, UNITS, TOTAL_COST, TOTAL_RETAIL, REF_NO_1, REF_NO_2, GL_REF_NO, OLD_UNIT_RETAIL, NEW_UNIT_RETAIL, PGM_NAME, SALES_TYPE, VAT_RATE, AV_COST, TIMESTAMP, REF_PACK_NO, TOTAL_COST_EXCL_ELC, POS_TRAN_ID, SALES_PROCESS_ID)
SELECT 
ITEM, DEPT, CLASS, SUBCLASS, PACK_IND, LOC_TYPE, LOCATION,TRAN_DATE+1,TRAN_CODE, ADJ_CODE, UNITS, TOTAL_COST, TOTAL_RETAIL, REF_NO_1, REF_NO_2, GL_REF_NO, OLD_UNIT_RETAIL, NEW_UNIT_RETAIL, PGM_NAME, SALES_TYPE, VAT_RATE, AV_COST, TRAN_DATA_TIMESTAMP+1 as TIMESTAMP, REF_PACK_NO, null as TOTAL_COST_EXCL_ELC,null,null  FROM IF_TRAN_DATA_BK_RETEN
where tran_code in ('22','25','20');
commit;

insert into tran_data_a (ITEM, DEPT, CLASS, SUBCLASS, PACK_IND, LOC_TYPE, LOCATION, TRAN_DATE, TRAN_CODE, ADJ_CODE, UNITS, TOTAL_COST, TOTAL_RETAIL, REF_NO_1, REF_NO_2, GL_REF_NO, OLD_UNIT_RETAIL, NEW_UNIT_RETAIL, PGM_NAME, SALES_TYPE, VAT_RATE, AV_COST, TIMESTAMP, REF_PACK_NO, TOTAL_COST_EXCL_ELC, POS_TRAN_ID, SALES_PROCESS_ID)
SELECT 
ITEM, DEPT, CLASS, SUBCLASS, PACK_IND, LOC_TYPE, LOCATION,TRAN_DATE+1 as TRAN_DATE,TRAN_CODE, ADJ_CODE, UNITS, TOTAL_COST, TOTAL_RETAIL, REF_NO_1, REF_NO_2, GL_REF_NO, OLD_UNIT_RETAIL, NEW_UNIT_RETAIL, PGM_NAME, SALES_TYPE, VAT_RATE, AV_COST, TRAN_DATA_TIMESTAMP+1 as TIMESTAMP, REF_PACK_NO, null as TOTAL_COST_EXCL_ELC,null,null  FROM IF_TRAN_DATA_BK_RETEN
where tran_code in ('87');
...
insert into rms.inv_adj
    select  ITEM,INV_STATUS,LOC_TYPE,LOCATION,ADJ_QTY ,REASON,l_inv_date,PREV_QTY,USER_ID ,ADJ_WEIGHT ,ADJ_WEIGHT_UOM,CREATE_ID ,l_inv_date 
        from rms.inv_adj inv1 where trunc(adj_date) = '09-NOV-21';

insert into rms.inv_status_qty
    select  ITEM, INV_STATUS, LOC_TYPE, LOCATION, QTY, l_inv_date, l_inv_date, LAST_UPDATE_ID 
        from rms.inv_status_qty inv1 where trunc(CREATE_DATETIME) = '09-NOV-21';
commit;
EXCEPTION
   when OTHERS THEN
...
select count(1) from rms.tran_data_a; --16 724 845

select distinct tran_date,count(1) from rms.tran_data_a group by tran_date order by 1; --
select distinct tran_date,count(1) from rms.tran_data_b group by tran_date order by 1; --80 58 150
