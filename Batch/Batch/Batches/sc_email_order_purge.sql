SELECT email_po_retention_days FROM SUPP_ASOS.sc_system_options;
...
WHERE
(0 < (NVL(MONTHS_BETWEEN(TO_DATE('20211110','YYYYMMDD'),
oh.close_date),0) - 25)));
...
select count(master_po_no) from SUPP_ASOS.sc_email_order_pubinfo where PUBLISHED_IND ='Y' and PROCESS_DATETIME is null;
        ...
        Update SUPP_ASOS.sc_email_order_pubinfo set process_datetime = SYSDATE - '31' where  order_no in (
            SELECT distinct order_no
              FROM SUPP_ASOS.ordhead oh
             WHERE 
             (0 < (NVL(MONTHS_BETWEEN(TO_DATE('20200121','YYYYMMDD'),
                     oh.close_date),0) - 21)) 
                     ); 
            commit;
