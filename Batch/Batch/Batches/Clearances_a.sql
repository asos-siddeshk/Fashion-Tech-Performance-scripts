    ...
    from rms.rpm_clearance rpc  where state like 'pricechange.state.approved' 
         and EFFECTIVE_DATE = '08-MAR-22' and ZONE_NODE_TYPE ='1' and skulist is not null and rownum<='200'
...
DECLARE
 ...
 cursor c_get_cuitem_clr is
    select  CLEARANCE_ID, 
            ...
            CHANGE_AMOUNT
    from rms.rpm_clearance rpc  where state like 'pricechange.state.approved' 
         and EFFECTIVE_DATE = '08-MAR-22' and ZONE_NODE_TYPE ='1' and skulist is not null and CHANGE_TYPE='2' and rownum<='200'
        and not exists (select 1 from ma_asos.ma_stage_clearance mspc where mspc.STAGE_CLEARANCE_ID =rpc.CLEARANCE_DISPLAY_ID); 
