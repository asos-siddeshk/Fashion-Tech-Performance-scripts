 select oh.master_order_no,
         oo.option_id,
         c.unit_cost
    from ma_asos.ma_stg_order oh,
         ...
         (select cd.item,
           ...
           where ch.cost_change          = cd.cost_change
             and cd.item                 = i.item
             and cd.item                 = isc.item
             and cd.supplier             = isc.supplier
             and ch.status               = 'A'
             and ch.cost_change_origin   = 'SUP'
             and i.item_level            < i.tran_level
             and trunc(ch.active_date)   = '16-JAN-24'
             and cd.recalc_ord_ind       = 'Y'
             and isc.primary_country_ind = 'Y'
             and isc.primary_supp_ind    = 'Y'
           group by cd.item,
                    cd.supplier,
                    isc.unit_cost) c
   where oh.master_order_no = oo.master_order_no
     and oh.supplier        = c.supplier
     and oo.option_id       = c.item
     and oh.status          in ('W', 'S')
     and exists (select 1
                   from ma_asos.ma_stg_order_option op
                  where op.master_order_no = oh.master_order_no
                    and op.option_id       = oo.option_id
                    and op.cost_source     = 'NORM'
                  union
                 select 1
                   from ma_asos.ma_stg_order_drops_detail od
                  where od.master_order_no = oh.master_order_no
                    and od.option_id       = oo.option_id
                    and od.cost_source     = 'NORM'
                  union
                 select 1
                   from ma_asos.ma_stg_order_item_dist oi
                  where oi.master_order_no = oh.master_order_no
                    and oi.option_id       = oo.option_id
                    and oi.cost_source     = 'NORM');
 
select vdate+1 from rms.period;

SELECT *  FROM rms.cost_susp_sup_head where ACTIVE_DATE ='08-MAR-22' and status = 'A';
SELECT *  FROM rms.cost_susp_sup_detail where COST_CHANGE in 
(SELECT COST_CHANGE  FROM rms.cost_susp_sup_head where ACTIVE_DATE ='08-MAR-22') and  RECALC_ORD_IND ='Y';

UPDATE cost_susp_sup_detail sEt RECALC_ORD_IND ='N'
where COST_CHANGE in (SELECT COST_CHANGE  FROM rms.cost_susp_sup_head where ACTIVE_DATE ='08-MAR-22') 
and RECALC_ORD_IND ='Y' and rownum <= '400';
