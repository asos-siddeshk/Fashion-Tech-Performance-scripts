...
SELECT distinct item,item_level FROM ITEM_MASTER IM
    WHERE EXISTS (SELECT 1 FROM ma_asos.MA_STG_UPLOAD_PROCESS_IDS  MID 
                                                where process_seq >= '197823'
...
SELECT distinct ITEM,dept FROM ITEM_MASTER IM 
   WHERE EXISTS (SELECT 1 FROM ma_asos.MA_STG_UPLOAD_PROCESS_IDS  MID where process_seq  >= '197823' AND MID.BUSINESS_OBJ_ID = IM.ITEM)) qur 
    WHERE not EXISTS (SELECT 1 FROM ITEM_LOC Il where Il.ITEM_PARENT = qur.ITEM);
...
SELECT count(distinct ITEM_PARENT) FROM ITEM_MASTER IM
    WHERE EXISTS (SELECT 1 FROM ma_asos.MA_STG_UPLOAD_PROCESS_IDS  MID 
                                                where process_seq  >= '197816'
                                                AND MID.BUSINESS_OBJ_ID = IM.ITEM);

SELECT distinct BUSINESS_OBJ_ID FROM ma_asos.MA_STG_UPLOAD_PROCESS_IDS  MID 
                                                where process_seq  >= '197816';
...
select * from ma_asos.MA_STG_ITEM_HEAD where item ='100747467';
...
select * from ma_asos.MA_STG_UPLOAD_PROCESS_LINE_IDS where process_seq = '197814';

select count(1) from ma_asos.MA_STG_UPLOAD_PROCESS_LINE_IDS where process_seq = '197814';

select count(1) from ma_asos.MA_STG_UPLOAD_PROCESS_LINE_IDS where process_seq >= '197816';

select * from ma_asos.MA_STG_UPLOAD_PROCESS where process_seq >= '197816';
select * from ma_asos.MA_STG_UPLOAD_PROCESS_LINE where process_seq >= '197816';
select * from ma_asos.MA_STG_UPLOAD_PROCESS_LINE_IDS where process_seq >= '197814';
select * from ma_asos.MA_STG_UPLOAD_BOUNDED_ATTR where process_seq >= '197814';
select * from ma_asos.MA_STG_UPLOAD_UNBOUNDED_ATTR where process_seq >= '197814';
...
SELECT DISTINCT IM.ITEM_PARENT FROM ITEM_MASTER IM
    WHERE ITEM IN (select DISTINCT iteM from rpm_stage_item_loc)
        AND EXISTS (SELECT 1 FROM ma_asos.MA_STG_UPLOAD_PROCESS_LINE_IDS  MID 
                                                where process_seq  BETWEEN '197814' AND '197814' 
