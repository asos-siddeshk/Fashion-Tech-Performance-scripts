...
select count(distinct(item_parent)) 
    ...
    item in (select item from repl_results where (ITEM, MASTER_ITEM) not in 
    (select item,master_item from repl_results_not) and PRIMARY_REPL_SUPPLIER ='1100000086' );
...
cursor c_loc is
   select wh from rms.wh where wh in ('1001','4001','3001','6001');
...
cursor c_rplatupd(l_loc rms.REPL_ATTR_UPDATE_LOC.loc%type) is
	...
	select im.item,
			...
			where im.item_level = '1'
					...
					where rownum<=200; -- Change for number of records
