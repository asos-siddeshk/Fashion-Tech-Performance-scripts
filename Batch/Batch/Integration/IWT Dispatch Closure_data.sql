...
DECLARE
  ...
  curr_seq   NUMBER(12);
BEGIN
 SELECT 7271452807 INTO last_used FROM dual; --
  LOOP
...
DECLARE
  ...
  k_tsf_no              rms.tsfhead.tsf_no%type        := '7271452807';
  ...
  CURSOR cur_wh IS 	
    select 	wh2.wh from rms.wh wh1, rms.wh wh2 
  	where 	wh1.org_unit_id!=wh2.org_unit_id and wh1.wh = n_from_loc  AND wh2.wh in ('4001','6001','3001')
    ORDER BY dbms_random.value; 
 

  CURSOR cur_item IS    
   select im.ITEM_ID, '8' as tsf_qty
...
DECLARE
  ...
  CURSOR cur_wh IS 	
    select 	wh2.wh from rms.wh wh1, rms.wh wh2 
  	where 	wh1.org_unit_id!=wh2.org_unit_id and wh1.wh = n_from_loc  AND wh2.wh in ('4001','6001','1001')
    ORDER BY dbms_random.value; 
 ...
 WHILE counter < num_rec LOOP
       ...
       for i in cur_wh loop
        ...
        END LOOP;
            ...
            L_RIB_XTsfDesc_REC.status         := 'A';
            L_RIB_XTsfDesc_REC.user_id        := 'PTUSER';
            L_RIB_XTsfDesc_REC.comment_desc   := 'iwtdispath_cls';
            L_RIB_XTsfDesc_REC.XTsfDtl_TBL    := l_RIB_XTsfDtl_TBL;
...
DECLARE
  ...
  CURSOR cur_wh IS 	
    select 	wh2.wh from rms.wh wh1, rms.wh wh2 
  	where 	wh1.org_unit_id!=wh2.org_unit_id and wh1.wh = n_from_loc  AND wh2.wh in ('3001','6001','1001')
    ORDER BY dbms_random.value; 
...
DECLARE
  ...
  CURSOR cur_wh IS 	
    select 	wh2.wh from rms.wh wh1, rms.wh wh2 
  	where 	wh1.org_unit_id!=wh2.org_unit_id and wh1.wh = n_from_loc  AND wh2.wh in ('3001','4001','1001')
    ORDER BY dbms_random.value; 
	...
	FOR k IN tsfship_item(l_tsf_no) LOOP
		...
		END LOOP;
            ...
            L_RIB_ASNOutCtn_TBL := "RIB_ASNOutCtn_TBL"();      
                ...
                L_RIB_ASNOutCtn_REC.comments              :=      'iwtdispath_cls:'||l_tsf_no; 
			...
			L_RIB_ASNOutDistro_TBL(L_RIB_ASNOutDistro_TBL.COUNT) := L_RIB_ASNOutDistro_REC; 
			      ...
			      L_RIB_ASNOutDesc_REC := "RIB_ASNOutDesc_REC"( 0 		-- rib_oid number
															...
															,'iwtdispath_cls:'||l_tsf_no		--comments
															,1 		    --carrier_code
