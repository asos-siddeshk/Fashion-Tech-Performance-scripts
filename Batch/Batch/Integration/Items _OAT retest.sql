select item from rms.item_master im where item_level ='1' and status ='A' and item = '101818952';

drop table oat_retest;

create table oat_retest as
select item from item_master im where CREATE_ID='ORACNV' and item_level ='1' and status ='A' and dept = '1013'
    and not exists (select 1 from skumar.processed_item pi where pi.item = im.item) and rownum <= '1500' order by item desc;

select * from oat_retest;

select count(1) from rms.item_master im where item in (select item from oat_retest);
select * from rms.ITEM_MFQUEUE where trunc(TRANSACTION_TIME_STAMP) = trunc(sysdate);
select * from rms.ITEMloc_MFQUEUE where trunc(TRANSACTION_TIME_STAMP) = trunc(sysdate);


select * from rms.item_master where ITEM = '100000394' or ITEM_PARENT = '100000394' or ITEM_GRANDPARENT= '100000394';
select * from rms.ITEM_MFQUEUE where item in (select item from rms.item_master where ITEM = '100000394' or ITEM_PARENT = '100000394' or ITEM_GRANDPARENT= '100000394');
select * from rms.ITEMloc_MFQUEUE where item in (select item from rms.item_master where ITEM = '100000394' or ITEM_PARENT = '100000394' or ITEM_GRANDPARENT= '100000394');

select * from rms.item_pub_info where item in (select item from rms.item_master where ITEM = '100000394' or ITEM_PARENT = '100000394' or ITEM_GRANDPARENT= '100000394');
...
declare
 ...
 Cursor c_get_item_parent is
      select item from rms.item_master im where item_level ='1'  and status ='A' and item in (select item from oat_retest);
...
create table oat_retest as
select item from item_master im where CREATE_ID='ORACNV' and item_level ='1' and status ='A' and dept = '1013'
    and not exists (select 1 from skumar.processed_item pi where pi.item = im.item) and rownum <= '500' order by item desc;

select * from oat_retest;

select count(1) from rms.item_master im where item in (select item from oat_retest) 
    or item_parent in (select item from oat_retest);
