...
select distinct INVOICEREFERENCE, PAYMENTREFERENCE, ORDERID,TO_CHAR(systimestamp,'YYYY-MM-DD"T"HH24:MI:SS:FF"Z"') AS Timestamp, STOCKREFERENCE,PaymentMethod from SKUMAR.CASHANDSALES_GST_AFS order by ORDERID;
select distinct INVOICEREFERENCE, PAYMENTREFERENCE, ORDERID,TO_CHAR(systimestamp,'YYYY-MM-DD"T"HH24:MI:SS:FF"Z"') AS Timestamp, STOCKREFERENCE,PaymentMethod from SKUMAR.CASHANDSALES_2022 order by ORDERID;
...
create table CASHANDSALES_2024_RET as 
select * from CASHANDSALES_2022  where ORDERID in ( 
    select * from (select distinct ORDERID from CASHANDSALES_2022) where rownum <= '20');

select distinct DISCOUNTTOTALAMOUNT, DISCOUNTCODE, DISCOUNTTYPE, DISCOUNTVALUE,TO_CHAR(systimestamp,'YYYY-MM-DD"T"HH24:MI:SS') AS timestamp,TO_CHAR(systimestamp,'YYYY-MM-DD"T"HH24:MI:SS:FF"Z"') AS createdDate  from SKUMAR.CASHANDSALES_2024_RET;
select distinct ORDERID, WLOCATION, STOCKREFERENCE  from SKUMAR.CASHANDSALES_2024_RET;
select distinct ORDERREFERENCE,DELIVERYCOUNTRYCODE, WLOCATION,IDORCONVERSIONID, CURRENCYCODE  from SKUMAR.CASHANDSALES_2024_RET;

select distinct INVOICEREFERENCE, PAYMENTREFERENCE, ORDERID,TO_CHAR(systimestamp,'YYYY-MM-DD"T"HH24:MI:SS:FF"Z"') AS Timestamp, STOCKREFERENCE,PaymentMethod from SKUMAR.CASHANDSALES_2024_RET order by ORDERID