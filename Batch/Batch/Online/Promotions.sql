...
declare
  ...
  stage_COMMIT  NUMBER(8)     := 0;
   ...
   L_PROMO_EVENT_ID             rms.rpm_stage_simple_promo.PROMO_EVENT_ID%type := 59;
    ...
    cursor cur_dept is
                select dept,class,subclass,zone_id,store  from (
                   select im.dept,im.class,im.subclass,ma.ZONE_ID,ma.store from skumar.item_master_op im, ma_asos.ma_pricing_defaults ma where 
                                (dept='1006' and ma.store = '20000') 
                                group by im.dept,im.class,im.subclass,ma.ZONE_ID,ma.store) where rownum <= '10' 
                                order by 1,2,3,4;
...
begin
     ...
     select  vdate+3 into l_promo_start_date from rms.period; 
     select  vdate+25 into l_promo_end_date from rms.period; 
     select  vdate+3 into l_dtl_start_date from rms.period; 
     select  vdate+25 into l_dtl_end_date from rms.period; 
...
Update rms.rpm_stage_simple_promo set PROCESS_ID =null, STATUS='N', ERROR_MESSAGE=null,STAGE_ID = rownum,PROMO_EVENT_ID='58';
...
INSERT into rpm_stage_simple_promo_bk2 select * from rpm_stage_simple_promo ;

delete from rpm_stage_simple_promo ;

select * from rpm_stage_simple_promo_bk2;

select PROMO_COMP_ID,count(1),LOCATION from 
    rpm_promo_item_loc_expl where PROMO_COMP_ID in (select distinct PROMO_COMP_ID from rpm_stage_simple_promo_bk2) group by PROMO_COMP_ID,LOCATION;

select PROMO_COMP_ID,count(1) from 
    rpm_promo_item_loc_expl where PROMO_COMP_ID in (select distinct PROMO_COMP_ID from rpm_stage_simple_promo) group by PROMO_COMP_ID;

select * from rpm_promo_item_loc_expl;
select * from rpm_promo_dtl;
select * from rpm_promo_event;

Insert into rpm_promo_event (PROMO_EVENT_ID,PROMO_EVENT_DISPLAY_ID,DESCRIPTION,THEME,START_DATE,END_DATE,LOCK_VERSION,CUST_ATTR_ID) values (60,'60','Promotion_PT_60','PERFORMANCE',to_date('01-JAN-23','DD-MON-RR'),to_date('31-DEC-23','DD-MON-RR'),null,null);
...
Update rpm_promo set PROMO_EVENT_ID ='60' where --PROMO_EVENT_ID = '21' and 
    PROMO_ID in (select distinct PROMO_ID from rpm_stage_simple_promo_bk3);


select * from rpm_promo_comp ;


SELECT  pe.PROMO_EVENT_ID,p.promo_display_id ,pe.PROMO_EVENT_DISPLAY_ID,pc.promo_comp_id,pc.comp_display_id,pc.promo_id,p.start_date,
pddl.change_type, pdtl.state, pc.type 
  FROM rpm_promo p,
       rpm_promo_event pe,
       rpm_promo_comp pc,
       rpm_promo_dtl pdtl,
...
WHERE  p.PROMO_EVENT_ID                 = pe.PROMO_EVENT_ID
  and  p.PROMO_EVENT_ID in ('58')
   ...
   --and rownum <= '500'
 GROUP BY pe.PROMO_EVENT_ID,pe.PROMO_EVENT_DISPLAY_ID,pc.promo_comp_id,pc.comp_display_id,pc.promo_id,p.promo_display_id,p.start_date, pddl.change_type, pdtl.state, pc.type 
order by pe.PROMO_EVENT_ID,pe.PROMO_EVENT_DISPLAY_ID,pc.promo_comp_id,pc.comp_display_id,pc.promo_id,p.promo_display_id,p.start_date,pddl.change_type, pdtl.state, pc.type ;


select node.item, decode(item_level, tran_level, 'TRAN', 'PARENT') item_type,
       NVL(rzl.location, rzl.zone_id) loc, rp.PROMO_EVENT_ID, rp.promo_display_id, comp.comp_display_id, comp.customer_type, 
...
--and rp.promo_display_id ='286460' 
   --and dtl.PROMO_DTL_ID in (select PROMO_DTL_ID from int_asos.int_pe_simple_promo_stg)
and rp.PROMO_EVENT_ID in ('58')
and dtl.promo_dtl_id = expl.promo_dtl_id(+)
order by node.item, rzl.location, approval_date desc, dtl.start_date;

select promo_id,PROMO_COMP_ID,count(1) from 
    rpm_promo_item_loc_expl where PROMO_COMP_ID in (select distinct PROMO_COMP_ID from rpm_stage_simple_promo_bk2) group by promo_id,PROMO_COMP_ID;
...
declare
    ...
    cursor cur_item is 
            select im.item from skumar.item_master_op im where 
               ...
               and not exists (select 1 from rms.rpm_stage_simple_promo rs where rs.item = im.item) and  rownum <= '20';
