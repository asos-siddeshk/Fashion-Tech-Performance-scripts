...
delete from int_asos.int_stg_man_tsf_upld where STATUS ='I';
...
select * from INT_ASOS.int_stg_man_tsf_upld where  status ='U';
select * from INT_ASOS.int_stg_man_tsf_upld where thread_no ='2';
select * from INT_ASOS.int_stg_man_tsf_upld where thread_no ='1';

select STATUS, THREAD_NO,count(1) from INT_ASOS.int_stg_man_tsf_upld where trunc(PROCESS_DATETIME) =trunc(sysdate) group by STATUS, THREAD_NO;
...
cursor c_tsf_upld  is
		select ITEM_ID,
			   ...
			   '6001' as TO_LOC,
			   1 as QUANTITY 
		from skumar.cust_tsf_upld ctu where FROM_LOC ='4001' and QUANTITY >='3' and
				not exists (select 1 from INT_ASOS.int_stg_man_tsf_upld ismt where ismt.item_id= ctu.ITEM_ID) and rownum<=1000;
