...
select state,count(1) from 
    rpm_clearance where trunc(EFFECTIVE_DATE)= '10-MAY-21' --and clearance_id in (select PRICE_EVENT_ID from rpm_bulk_cc_pe_thread)
group by state; --480

--  09-MAY-21	60 // 10-MAY-21	60
...
select * from ma_asos.ma_stage_clearance;
select * from period;
select state,count(1) from rms.rpm_clearance where EFFECTIVE_DATE between '10-JAN-19' and '31-JAN-19' group by state; -- 5008
select EFFECTIVE_DATE,count(1) from rms.rpm_clearance where EFFECTIVE_DATE between '10-JAN-19' and '31-JAN-19'  and state ='pricechange.state.approved' group by EFFECTIVE_DATE order by 1; -- 5008

Update ma_asos.ma_stage_clearance set effective_date='02-MAR-20' where status ='N' ;

delete from rms.rpm_stage_clearance;
delete from ma_asos.ma_stage_clearance where status ='N' and trunc(effective_date)>='02-MAR-20' and rownum ;
...
select sh.SKULIST from rms.skulist_head sh where sh.sKULIST_DESC like '500Clearance%'
    ...
    and not exists (select 1 from rms.rpm_clearance rc where rc.EFFECTIVE_DATE between '20-DEC-2018' and '31-MAY-2022'
    and rc.SKULIST = sh.SKULIST) ;
   ...
   l_subclass                	    rms.subclass.subclass%type; 
	 ...
	 cursor c_get_cuitem_clr (p_zone_id ma_asos.ma_stage_clearance.zone_id%type)  is
            select sh.SKULIST from rms.skulist_head sh where sh.sKULIST_DESC like '500Clearance%'
                ...
                and not exists (select 1 from rms.rpm_clearance rc where rc.EFFECTIVE_DATE between '20-DEC-2018' and '31-MAY-2022'
                         and rc.SKULIST = sh.SKULIST and rc.zone_id = p_zone_id) and rownum <='1';
...
for m in 0..1 loop
    ...
    select vdate+7+m into p_out_of_stock_date from rms.period;
    select vdate+7+m into p_reset_date from rms.period;
...
(select skulist from rms.skulist_head where sKULIST_DESC like '500Clearance%') group by SKULIST having count(1)<= '100';
	 ...
	 cursor c_get_cuitem_clr (l_dept rms.subclass.dept%type,l_class rms.subclass.class%type,l_subclass rms.subclass.subclass%type,
                ...
                select distinct im.item,ma.zone_id,il.loc as location,il.selling_unit_retail as selling_unit_retail
                     ...
                     --    and not exists (select 1 from rms.item_loc il where il.item_parent = im.item and (il.promo_retail is null or il.CLEAR_IND ='Y') and il.loc = l_location and rownum <= '1')
                         ...
                         and not exists (select 1 from ma_asos.ma_stage_simple_promo msp where msp.item = im.item  and msp.location = il.loc) 
...
for j in 0..5 loop
    ...
    select vdate+1+m into p_effective_date from rms.period;
    select vdate+2+m+j into p_out_of_stock_date from rms.period;
    select vdate+2+m+j into p_reset_date from rms.period;
 ...
 stage_COMMIT  NUMBER(8)     := 0;
   ...
   l_subclass               rms.subclass.subclass%type; 
	...
	select dept,class,subclass,zone_id  from (
		   select distinct im.dept,im.class,im.subclass, ma.ZONE_ID from rms.subclass im, ma_asos.ma_pricing_defaults ma 
            group by im.dept,im.class,im.subclass, ma.ZONE_ID ) 
            where rownum<= '1000' order by 1,2,3,4;
    ...
    CURSOR c_get_cuitem_clr (l_dept rms.subclass.dept%type,l_class rms.subclass.class%type,l_subclass rms.subclass.subclass%type,l_zone_id rms.rpm_stage_simple_promo.zone_id%type)is
            select      im.item,
				...
				where il.item             = im.item
				 ...
				 and not exists (select 1 from ma_asos.ma_stage_clearance mpc  where mpc.item = im.item and mpc.location = ma.store)
                and not exists (select 1 from ma_asos.ma_stage_clearance mpc  where mpc.item = im.item and mpc.location = ma.store)
                	  and rownum<=5
                  order by item,
						zone_id, 
