...
l_sp_stage_id                           ma_asos.ma_stage_simple_promo.STAGE_SIMPLE_PROMO_ID%type;
 ...
 cursor c_get_cuitem_sp is
			 select rp.PROMO_ID, rp.PROMO_DISPLAY_ID, rp.NAME as promo_name,rp.PROMO_EVENT_ID, rp.START_DATE as PROMO_START_DATE, rp.END_DATE as PROMO_END_DATE, rpc.PROMO_COMP_ID, rpc.COMP_DISPLAY_ID
				   ...
				   ,rpdmn.ITEM,rpzl.ZONE_NODE_TYPE, rpzl.ZONE_ID, rpzl.LOCATION,rpddl.CHANGE_TYPE, rpddl.CHANGE_AMOUNT, rpddl.CHANGE_CURRENCY as CURRENCY_CODE, rpddl.CHANGE_SELLING_UOM
...
BEGIN
FOR cust_ma in c_get_cuitem_sp Loop
 ...
 stage_COMMIT := stage_COMMIT+1;
	   IF MOD(stage_COMMIT, 100) = 0 THEN
		commit;
       END IF;
