...
SELECT count(1) FROM v_restart_cost_chg rv,
             cost_susp_sup_head h
       WHERE h.status        = 'A'
         AND TRUNC(h.active_date) = TO_DATE('20210509', 'YYYYMMDD')
...
update cost_susp_sup_head h set status = 'R'
       WHERE h.status        = 'A'
         AND TRUNC(h.active_date) = TO_DATE('20200302', 'YYYYMMDD') and rownum <= '8000';
