...
update SUPP_ASOS.sc_event_message set MAX_ATTEMPTS = '6';
select * from SUPP_ASOS.SC_EVENT_MESSAGE_FAILURE;

"java.net.ConnectException: Tried all: '1' addresses, but could not connect over HTTP to server: 'asnin-vnp2.services.kingsway.asos.com', port: '80'
 failed reasons: [0] address:'asnin-vnp2.services.kingsway.asos.com/10.68.128.29',port:'80' : java.net.ConnectException: Connection timed out (Connection timed out)"

delete from SUPP_ASOS.SC_ORDER_LOCK;\
commit;

select count(1) from SUPP_ASOS.item_master where item_level = '1';
select count(1) from SUPP_ASOS.tsfhead;
...
SELECT 53106235399+ 900000 from dual;
...
BEGIN
  SELECT 53107135399 INTO last_used FROM dual;
...
select * from SUPP_ASOS.SC_SUPP_USERS_CFG where USERNAME like 'PTESTUSER%';


delete from SUPP_ASOS.SC_SUPP_USERS_CFG where USERNAME like 'PTESTUSER%' and SUPP_ID! = '1100000086';
