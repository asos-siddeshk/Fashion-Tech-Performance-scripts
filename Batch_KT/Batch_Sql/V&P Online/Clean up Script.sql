...
delete from ma_asos.ma_price_change where trunc(CREATE_DATETIME)> = '10-JUN-21' AND CREATE_ID likE 'PTESTUSER%';


select status,count(1) from ma_asos.ma_price_change where trunc(CREATE_DATETIME)> = '10-JUN-21' group by status;
select status,EFFECTIVE_DATE,count(1) from ma_asos.ma_price_change where trunc(CREATE_DATETIME)> = '10-JUN-21' group by status,EFFECTIVE_DATE order by 2,1; --
select EFFECTIVE_DATE,count(1) from ma_asos.ma_stage_price_change group by EFFECTIVE_DATE order by 1; --
select * from ma_asos.ma_stage_price_change where trunc(CREATE_DATETIME)> = '10-JUN-21';
delete from ma_asos.ma_price_change where RMS_PRICE_CHANGE_ID in (select PRICE_CHANGE_ID from ma_asos.ma_stage_price_change where status='N');
delete from ma_asos.ma_stage_price_change where status='N';
delete from ma_asos.ma_price_change where trunc(CREATE_DATETIME)> = '10-JUN-21' AND CREATE_ID likE 'PTESTUSER%';
select * from ma_asos.ma_price_change where trunc(CREATE_DATETIME) = '10-JUN-21';
...
DECLARE
  ...
  CURSOR c_order is
   ...
   UNION
    select distinct MASTER_ORDER_NO from ma_asos.ma_stg_sizing_sku where MASTER_ORDER_NO >='25579853' ;
...
select master_po_no,count(*) from rms.ordhead where CREATE_DATETIME>= to_date('21-SEP-2020 17:00', 'DD-MON-YYYY hh24:mi') group by master_po_no;
...
select * from  SVC_ORDHEAD where master_po_no in (select master_po_no from rms.ordhead where CREATE_DATETIME>= to_date('21-SEP-2020 17:00', 'DD-MON-YYYY hh24:mi'));
select * from  coresvc_po_err where order_no IN (select order_no from  SVC_ORDHEAD where master_po_no in (select master_po_no from rms.ordhead where CREATE_DATETIME>= to_date('21-SEP-2020 17:00', 'DD-MON-YYYY hh24:mi')));
...
ALTER INDEX MA_STG_SIZING_SKU_I2 REBUILD ONLINE;
