...
select * from asn_cre a where not exists (select 1 from shipment sh where sh.order_no = a.order_no);
    ...
    select master_po_no,order_no,status,PICKUP_DATE as expected_handover,
        ...
        LATEST_SHIP_DATE as  expected_handover_end,
        NOT_BEFORE_DATE as expected_shipment_date,
        NOT_AFTER_DATE as expected_Delivery_date from ordhead where order_no in (select order_no from 
            asn_cre where order_no between 50000635826 and 50000636768);

create table asn_cre as select order_no from ordhead where 1=2;
delete from asn_cre a where exists (select 1 from shipment sh where sh.order_no = a.order_no);
select * from asn_cre a where not exists (select 1 from shipment sh where sh.order_no = a.order_no);
...
DECLARE
        i_need_date             	date;
        i_vdate                 	date  := '15-MAR-2020';
        i_vdate1                 	date  := '15-MAR-2020';
        v_return                	boolean;
  ...
  cursor cur_Order is
        select order_no from asn_cre where status is null and rownum <= '20' order by 1;
...
--	for j in 0..0 loop 
	...
	for m in cur_order loop 
        ...
        select I_VDATE into i_vdate1 from dual;
                counter   := counter + 1; 

            
              
            IF MOD(counter, 25) = 0 THEN
              I_VDATE   := I_VDATE + 1; 
               --commit;
              END IF;
