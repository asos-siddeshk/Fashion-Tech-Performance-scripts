create table orditemloc as 
    select oh.order_no,od.item,od.location,oh.supplier 
        from ordloc od ,ordhead oh where oh.order_no = od.order_no and oh.status ='S';
...
select  * from (
select distinct DIVISION, DEPT, CLASS, SUBCLASS,count(distinct(order_no)) as count 
    from orditemloc_d group by DIVISION, DEPT, CLASS, SUBCLASS having count( distinct (order_no)) > 2000 order by count(distinct(order_no)) ) where rownum <= '10';
...
select distinct DIVISION, DIV_NAME, DEPT, DEPT_NAME,CLASS, CLASS_NAME,SUBCLASS, SUB_NAME from V_MERCH_HIERARCHY where dept = '1158' and class = '7' and division = '1' order by SUB_NAME;
