...
select * from rms.skulist_head where sKULIST_DESC like 'Price Change%';
...
select DEPT, CLASS, SUBCLASS,count(distinct(PRICE_CHANGE_ID))  as counts
    from pricemasssearch group by DEPT, CLASS, SUBCLASS having count( distinct (PRICE_CHANGE_ID)) > 2000 order by count(distinct(PRICE_CHANGE_ID))
    ) where rownum<= '5';
